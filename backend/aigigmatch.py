...
def assign_task_to_worker(task_id, worker_name):
    conn = get_database_connection()
    try:
        with conn.cursor() as curs:
            curs.execute("""
            ...
            """, (worker_name, task_id))
            conn.commit()
        logging.info(f"Task {task_id} assigned to worker {worker_name}")
        return f"Task {task_id} assigned to worker {worker_name}"
    except Exception as e:
        logging.error(f"Error assigning task to worker: {e}")
        conn.rollback()
        return f"Error assigning task to worker: {e}"
    finally:
        conn.close()
...
def fetch_tasks_by_user(user_id):
    ...
    try:
        with conn.cursor() as curs:
            curs.execute("""
                SELECT task_id, task_description, additional_info, status, assigned_to, result_url
        ...
        logging.error(f"Error fetching tasks by user: {e}")
        raise
    finally:
        conn.close()
    return tasks

def fetch_tasks_by_worker(worker_name):
    conn = get_database_connection()
    tasks = []
    try:
        with conn.cursor() as curs:
            curs.execute("""
                SELECT task_id, task_description, additional_info, status, result_url
        ...
        logging.error(f"Error fetching tasks by worker: {e}")
        raise
    finally:
        conn.close()
    return tasks

def worker_login(worker_name):
    conn = get_database_connection()
    try:
        with conn.cursor() as curs:
            curs.execute("SELECT 1 FROM gig_workers WHERE name = %s", (worker_name,))
            return curs.fetchone() is not None
    except Exception as e:
        logging.error(f"Error during worker login: {e}")
        return False
    finally:
        conn.close()
...
def gradio_interface():
    """
    Starts the Gradio interface for the TaskNinja application.

    This function sets up the Gradio interface with various tabs and components for different functionalities of the TaskNinja application. It includes tabs for AI interaction, task management, and user management.
...
def gradio_interface():
    ...
    with gr.Blocks(css=".gradio-container {background-color: #f4f4f4}") as iface:
        ...
        ai_tab = gr.TabItem("AI Interaction", visible=True)
        with gr.Tabs():
            # Existing tabs here...
            ...
            with gr.TabItem("User Management"):
                ...
                # Fetch User Tasks
                fetch_user_tasks_id = gr.Number(label="User ID")
                fetch_user_tasks_button = gr.Button(value="Fetch User Tasks")
                fetch_user_tasks_output = gr.JSON(label="User Tasks")

                def handle_fetch_user_tasks(user_id):
                    return fetch_tasks_by_user(user_id)
            ...
            with gr.TabItem("Ninja Management"):
                ...
                # Worker Login
                worker_login_name = gr.Textbox(label="Worker Name")
                worker_login_button = gr.Button(value="Login")
                worker_login_output = gr.Textbox(label="Login Result")

                def handle_worker_login(worker_name):
                    if worker_login(worker_name):
                        return "Login successful"
                    else:
                        return "Login failed"

                worker_login_button.click(handle_worker_login, inputs=worker_login_name, outputs=worker_login_output)

                # Fetch Worker Tasks
                fetch_worker_tasks_name = gr.Textbox(label="Worker Name")
                fetch_worker_tasks_button = gr.Button(value="Fetch Worker Tasks")
                fetch_worker_tasks_output = gr.JSON(label="Worker Tasks")

                def handle_fetch_worker_tasks(worker_name):
                    return fetch_tasks_by_worker(worker_name)

                fetch_worker_tasks_button.click(handle_fetch_worker_tasks, inputs=fetch_worker_tasks_name, outputs=fetch_worker_tasks_output)
